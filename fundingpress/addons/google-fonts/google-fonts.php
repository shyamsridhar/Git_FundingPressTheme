    ...
    class googlefonts {
        ...
        */
        ...
        function gf_get_font_file(){
            ...
            if($fonts_object && is_object($fonts_object)){
                if(isset($fonts_object->error) && $fonts_object->error){
                    $this->gf_notices[] = sprintf(esc_html__('Google API Error: %s. %s', 'fundingpress'), $fonts_object->error->code, $fonts_object->error->message);
                }
            ...
            if($this->gf_fonts){
                ...
                if(time() >= ($filetime + (60 * 60 * 12))){
                    if($this->gf_update_font_cache()){
                        $this->gf_notices[] = sprintf(esc_html__("Font list sync successful, updated %s.", 'fundingpress'), $this->gf_font_list_time());
                        $result = true;
                    }else{
                        $this->gf_notices[] = sprintf(esc_html__("Unable to do a live update on the font list. Using cached version from %s.", 'fundingpress'), $this->gf_font_list_time($filetime));
                    }
                }else{
                    $this->gf_notices[] = sprintf(esc_html__("Font list is up to date. Last updated %s.", 'fundingpress'), $this->gf_font_list_time($filetime));
                }
            }else{
                if($this->gf_update_font_cache()){
                    $this->gf_notices[] = sprintf(esc_html__("Font list sync successful, created %s.", 'fundingpress'), $this->gf_font_list_time());
                    $result = true;
                }else{
                    $this->gf_notices[] = esc_html__("Font list file doesn't exist and wasn't able to be updated.", 'fundingpress');
                }
            }
        ...
        function gf_download_font_list($url){
            ...
            if(function_exists('wp_remote_get')){
                ...
                if( is_wp_error($response)){
                    ...
                    $this->gf_notices[] = sprintf(esc_html__("Unable to connect to Google's Webfont server at <a href='%s' target='_blank'>this URL</a>.", 'fundingpress'), $url);
                    ...
                    if(isset($response['body']) && $response['body']){
                        ...
                        if(strpos($response['body'], 'error') === false){
                            ...
                            $this->gf_notices[] = '<span class="slight">' . sprintf(esc_html__('Google API Notice: %s. %s', 'fundingpress'), $error->error->code, $error->error->message) . '</span>';
        ...
        function gf_get_local_fonts(){
            ...
            if($fonts){
                $this->gf_notices[] = esc_html__("Using the local font list file because we could not connect with Google.", 'fundingpress');
            }else{
                $this->gf_notices[] = esc_html__("Local font list file cannot be found or does not exist.", 'fundingpress');
            }
        ...
        function gf_get_fonts_select($name="googlefont"){
            ...
            if($this->gf_fonts && is_array($this->gf_fonts)){
                ...
                foreach($this->gf_fonts as $font){
                    ...
                    if($first){
                        ...
                        $out.="<option class='".implode(" ",$class)."' value='off' ".
                                ...
                                ">" . esc_html__("None (Turn off this font)", 'fundingpress') . "</option>\n";
                    }
                ...
                if($current_selection && $current_selection != 'off'){
                    $out .= '<a href="#'.$name.'-select" class="show_hide" id="show_hide-'.$name.'">' . esc_html__('Show Options', 'fundingpress') . '</a>';
                }
                ...
                $out.='<div style="clear:both;"><input class="button-primary" type="submit" name="googlefonts_save" value="' . esc_html__('Save All Fonts', 'fundingpress') . '" /></div>';
            ...
            $out.= "<div class='webfonts-selections'>\n";
                ...
                <?php
                    ...
                    $normal_name = $this->options['googlefont_selections'][$name]['family'];
                        ...
                        $out.= "<div class='webfonts-preview'><h3>".esc_html__('Preview:', 'fundingpress')."</h3>\n";
                ...
                $out.= "<div class='webfonts-variants'><h3>" . esc_html__('1. Choose the font styles you want:*', 'fundingpress') . "</h3>\n".$variants."</div>\n";

                /* add in the dom elements the user would like it to affect and custom css box */
                $out.= "<div class='webfonts-usage'><h3>" . esc_html__('2. Elements you want to assign this font to:*', 'fundingpress') . "</h3>\n".$this->gf_get_usage_checkboxes($name)."</div>\n";
                $out.= "<div class='webfonts-css'><h3>" . esc_html__('3. Custom CSS (optional):', 'fundingpress') . "</h3>\n<textarea name='".$name."[css]' id='".$name."_css'>".stripslashes($this->options[$name."_css"])."</textarea>\n</div>\n";

                /* add in subsets */
                $out.= "<div class='webfonts-subsets'><h3>" . esc_html__('4. Choose character sets you want.', 'fundingpress') . "</h3>\n".$subsets."</div>\n";
            $out.="</div>";
            return $out;
        ...
        function gf_get_selection_boxes($element_names){
            ...
            if(is_array($element_names)){
                ...
                foreach($element_names as $name){
                    $out.="<h2>" . sprintf(esc_html__('Font %s', 'fundingpress'), $i) . "</h2>\n";
        ...
        */
        function filter_plugin_actions($links, $file) {
           ...
           $settings_link = '<a href="admin.php?page=' . basename(__FILE__) . '">' . esc_html__('Settings', 'fundingpress') . '</a>';
        ...
        function gf_handle_submission($data){
            if (! wp_verify_nonce($_POST['_wpnonce'], 'googlefonts-update-options') ) die(esc_html__('Whoops! There was a problem with the data you posted. Please go back and try again.', 'fundingpress'));
        ...
        function admin_options_page() {
            ...
            if(isset($_POST['googlefonts_save']) && $_POST['googlefonts_save']){
                if($this->gf_handle_submission($_POST)){
                    $message = '<div class="updated"><p>' . esc_html__('Success! Your changes were sucessfully saved!', 'fundingpress') . '</p></div>';
                }else{
                    $message = '<div class="error"><p>' . esc_html__('Error. Either you did not make any changes, or your changes did not save. Please try again.', 'fundingpress') . '</p></div>';
                }
            }
            ?>
            <div class="wrap">
                <table width="650" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td>
                        ...
                        <h1><?php esc_html_e('Google Font Control Panel', 'fundingpress'); ?></h1>
                        ...
                        <h2><?php esc_html_e('Select Fonts', 'fundingpress');?></h2>
