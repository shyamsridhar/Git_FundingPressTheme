...
class SmartMetaBox {
	...
	public function show($post) {
		...
		echo '<input type="hidden" name="' . esc_attr($this->id) . '_meta_box_nonce" value="', wp_create_nonce('smartmetabox' . $this->id) , '" />';
		...
		foreach ($this->meta_box['fields'] as $field) {
			...
			echo '<tr>', '<th style="width:10%"><label for="', $id, '">', $name, '</label></th>', '<td>';
