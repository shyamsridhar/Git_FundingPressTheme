...
require_once(dirname(__FILE__).'/lib/paypal/PPBootStrap.php');

/**
 * An implementation of the payment interface for PayPal.
 */
$pp_confs = Configuration::getAcctAndConfig();
...
class F_PayPal{
	...
	if($f_paypal['mode'] =='sandbox'){
        ...
        $preapproval->memo = sprintf(esc_html__("Funding \"%s\", from \"%s\", for the reward \"%s\", if we reach our target.", 'fundingpress'), $project->post_title, get_bloginfo('name'), $reward->post_title);
        ...
        $preapproval->maxNumberOfPayments = 2;
        ...
        $adaptivePayment = new AdaptivePaymentsService($pp_confs);
    ...
    if($f_paypal['mode'] =='sandbox'){
        ...
        $per =  $f_paypal['admin-commission'];
        $totadminamount = $per /100;
        $receiver->email = $f_paypal['email'];
        $receiver->amount = floatval($totadminamount*$amm );

        $receiver1 = new receiver();
        $receiver1->email = $admin_info->paypal_email;
        $receiver1->amount = floatval($amm - ($totadminamount*$amm));
        ...
        $adaptivePayment = new AdaptivePaymentsService($pp_confs);
          ...
          elseif(strtoupper($response->responseEnvelope->ack) == 'SUCCESS') {
