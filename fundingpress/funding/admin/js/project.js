jQuery(function($){
	...
	$( "input[name=f_target_date]" ).datepicker({minDate: '0', dateFormat: formatdatuma });
	...
	$('#add-reward-save').click(function(){
		...
		if(editing == false){
			...
			if(newReward.title ==  '') newReward.title = 'Title';
	...
	$('#collect-funding').click(function(){
		if(confirm('Are you sure you want to collect funding?')){
			...
			var displayDonationModal = function(){
				...
				$('#f-modal-donate input[name=currency_code]').val(fundingpress.currency);
			...
			var displayDonationModal2 = function(){
				...
				$('#f-modal-donate2 input[name=currency_code]').val(fundingpress.currency);
