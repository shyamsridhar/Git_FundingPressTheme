<div id="project-funders">
	<?php foreach($funders as $funder) : ?>
		...
		<div class="funder" data-charged="<?php !empty($charged) ? 'true' : 'false' ?>" data-funder-id="<?php print $funder->ID ?>" data-project-id="<?php print $project->ID ?>">
			<div class="avatar">
				<a href="mailto:<?php print $funder_info['email'] ?>" title="<?php printf(esc_html__('Email %s', 'fundingpress'), $funder_info['name']) ?>">
	...
	<p>
		...
		<a href="<?php print wp_nonce_url(add_query_arg(array('fa' => 'export_funders', 'project' => $post->ID), site_url()), 'export_funders') ?>" target="_blank"><?php esc_html_e("Download", 'fundingpress') ?></a>
		<p class="description"><?php esc_html_e("You can collect funding after you've reached your target", 'fundingpress') ?></p>
	</p>
</div>