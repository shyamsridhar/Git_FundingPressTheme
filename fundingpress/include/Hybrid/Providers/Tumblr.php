...
class Hybrid_Providers_Tumblr extends Hybrid_Provider_Model_OAuth1
{
		...
		try{  
			...
			foreach ( $profile->response->user->blogs as $blog ){
				if( $blog->primary ){
					...
					$this->user->profile->webSiteURL	= $blog->followers;
	...
	*/ 
	...
	{
		$holder = explode ("_-*-_", $status);
		$parameters = array( 'type' => "photo", 'caption' => $holder[0], 'link' => $holder[1], 'source' => $holder[2] ); 
		$response  = $this->api->post( "blog/" . $this->token( "primary_blog" ) . '/post', $parameters );  
		//echo "blog/" . $this->token( "primary_blog" ) . '/post';
		//print_r ($this);
