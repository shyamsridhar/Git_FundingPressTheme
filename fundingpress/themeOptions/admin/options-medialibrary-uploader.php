<?php
/**
 * Media Library-driven AJAX File Uploader Module (2010-11-05)
 *
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . esc_html__( 'Upload' , 'fundingpress') . '" rel="' . $int . '" />' . "\n";
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = esc_html__( 'View File', 'fundingpress' );
...
if ( ! function_exists( 'optionsframework_mlu_get_silentpost' ) ) {
	function optionsframework_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			$post = get_page_by_title( $_title, OBJECT, 'optionsframework');
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( esc_html__( 'Gallery', 'fundingpress' ), esc_html__( 'Previously Uploaded', 'fundingpress' ), $tabs['gallery'] );
		return $tabs;
	}
}
